...
public class CheckVersionUtils {
    ...
    private static void downApp(final Context context, final String appName, final String appLine) {
        File cfile = new File(FileUtils.getSDPath(), appName);
            ...
            public void run() {
                ...
                try {
                    ...
                    if(connection.getResponseCode()==HttpURLConnection.HTTP_OK) {
                        ...
                        final File cfile = new File(FileUtils.getSDPath(), appName);
                        //如果不存在则新建文件
                        if (!cfile.exists()) {
                            cfile.createNewFile();
                        }
                        ...
                        //失败
                        com.topine.www.recruit.util.ToastUtils.showToast("发生网络错误了");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                    com.topine.www.recruit.util.ToastUtils.showToast("发生未知错误了");
                }
            }
        });
    }
